...
function longestName(array) {
  ...
  for (const obj of array) {
    Object.keys(obj.name).forEach(function (key) {
      fullName += obj.name[key];
    });
    if (fullName.length > count) {
      maxIndex = array.indexOf(obj);
...
// runTest(longestName(bucketOfSloths), bucketOfSloths[0]);
// runTest(longestName(bucketOfSloths.slice(1)), bucketOfSloths[2]);
// runTest(longestName(), undefined);

//2
//関数 getSortedByAge を宣言してください。

/**
 * @param {Array<object>} ??? - ナマケモノの情報が入った配列
...
//       if (arrayCopy[j].age < arrayCopy[j - 1].age) {
//         let tmp = arrayCopy[j];
...
function getSortedByAge(array) {
  const arrayCopy = array.slice();
  return arrayCopy.sort((a, b) => a.age - b.age);
}
...
// runTest(getSortedByAge(bucketOfSloths), bucketOfSlothsSortedByAge);

// 元の bucketOfSloths 配列が変更されていないことを確認する。
// runTest(JSON.stringify(bucketOfSloths), bucketOfSlothsCopy);
